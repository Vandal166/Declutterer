...
public sealed class DeleteService : IDeleteService
{
    ...
    public async Task<DeleteResult> DeleteItemsAsync(ObservableCollection<TreeNode> items, DeleteMode mode,
        IProgress<DeleteProgress>? progress = null,
        CancellationToken cancellationToken = default)
    {
        return mode == DeleteMode.RecycleBin
            ? await MoveToRecycleBinAsync(items, progress, cancellationToken)
            : await DeletePermanentlyAsync(items, progress, cancellationToken);
    }
